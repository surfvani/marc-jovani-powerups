...
- **Last updated:** 2026-06-10 (added Step 9.5 — plain-English sanity check: after printing the handoff prompt, agent must explain in 3-5 bullet points what it will make the next agent do, flag any drift from the user's chosen scoping option, and wait for approval before ending the session. Fixes the known failure mode where the handoff prompt goes creative and sends the next agent in the wrong direction). Prior: 2026-05-24 (added)
