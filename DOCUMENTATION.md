...
│   ├── CLAUDEAPPCCOPERATOR.md  ← → ~/.claude/personas/CLAUDEAPPCCOPERATOR.md — operating app_cc (dev → GitHub → prod, prod Postgres) over ssh from this box; loadable standalone or read by another persona on Marc's say-so
...
└── skills/                     ← every Claude Code skill in this collection
    ...
    ├── makemesee/SKILL.md                  ← frame-opening explainer for things foreign to Marc's lived experience (7-move discipline: blocking-assumption flip → steelman → bridges → epistemic marking → spectrum → belief audit → the scene; sovereignty close)
...
- **Source:** fresh-authored 2026-08-30 (not a verbatim port). Extracted from two conversations that landed for the same mechanical reasons: the hunting-culture explanation ("I just have a hard time understanding how killing something that's alive brings joy... I just want to understand", 30 Aug session on lake-vault) and the manager's operator/COO/CEO explanations ("quiero como VER esa realidad que no he experimentado", 29–30 Aug manager session). Marc's ask, verbatim: *"an skill that I can use any time I need this type of mind opening awareness through educated answers"* — option chosen: skill + recommendation note inside the CLAUDEMANAGER persona.
- **Trigger description:** see frontmatter `description:` field in `skills/makemesee/SKILL.md`
- **Purpose:** Frame-opening explanations — for when Marc wants to UNDERSTAND something foreign to his lived experience (a culture/worldview, an unfamiliar role or structure, an unlived reality), not simplify it and not execute anything. Runs the 7-move discipline in order: ① find the ONE blocking assumption and flip it first ② steelman from inside (test: an insider would say "yes, that's us") ③ bridge every abstract piece to Marc's lived world (analogies marked AS analogies) ④ mark epistemic status — documented fact / what they report / interpretation (no-fabrication outranks a satisfying answer) ⑤ show the internal spectrum where insiders disagree (validates partial gut feelings, kills the fake agree/disagree binary) ⑥ when the block is a limiting belief, audit it with HIS own data and history ⑦ paint the scene — make the unlived visible, labeled pattern-not-promise. Mandatory close: sovereignty — understanding ≠ agreeing, never selling ("si no es la panacea... no me la vendas"). Register: house defaults but allowed to breathe (400–800 words of connected prose under clear headers — frame-building needs it; this is deliberately NOT /simplll keyword compression). Distinct from /simplll (compresses what he already understands), /claudeclarity (reader-facing docs), and any execution skill.
- **Companion note:** the CLAUDEMANAGER persona's "Skills & Tools You Use" table gained a pointer row the same day (server-specific/CLAUDEMANAGER.md) so the manager invokes the skill instead of reimplementing it.
- **Last updated:** 2026-08-30 (created)
