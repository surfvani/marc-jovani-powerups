...
- Board **MAÑANA bar** + **`dia-card.html`** → redeploy the "EL DÍA" artifact, SAME url (https://claude.ai/code/artifact/bf758d00-8d2a-4cec-9f1e-22ba5160b12f — rebuilt 18 Aug 2026 under Marc's new account; the old `74279c94-37ac-474d-8144-45f0c4cfdcc6` is dead). URGENCIA items carry the ultra-mini red dot; INTENTIONAL items stay clean. **The FLOW line carries design WEIGHT and a CONCRETE HOUR** — larger type, the hour in mono accent, "no negociable" (Marc, 17 Aug: *"el flow hay que schedulearlo — si no se schedulea, me lo salto… ocurre a esta hora, no es negociable; todo lo demás ocurre alrededor"*). Footer stamp updated (`date` first).
