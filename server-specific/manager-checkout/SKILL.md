...
- Board **MAÑANA bar** + **`dia-card.html`** → redeploy the "EL DÍA" artifact, SAME url (https://claude.ai/artifact/2TYVCSqbKWeLN1bo1rq2Vf — id 0bcde176-9c83-4c8e-bd8c-931676cf25b2 — ⟳ 18 Sep 20:5x: login → mjovani87@gmail.com, repointed to this card (v31) · ⟳ 14 Sep 2026: the login switched again at 21:09 (Marc's rotate) → 24566e64 returned NOT FOUND and `Artifact list` showed b6480e3a (a FOURTH card, the 20 Aug build) as this login's only EL DÍA → repointed, no rebuild; the boot turn's republish was denied by the auto-mode classifier, the brief hop republishes. ⟳ 13 Sep 2026: the login switched again at 16:18 → bf758d00 returned NOT FOUND and `Artifact list` showed 24566e64 (the 27 Aug build) live → EL DÍA is 24566e64 on this login, republished v2. ⟳ 12 Sep 2026: the login switched SÁB ~15:19 (mrcjovani@gmail.com) → bf758d00 (the 18 Aug build) is the CURRENT login's live EL DÍA, republished v12; ⟳ 11 Sep 2026: 0bcde176 was that login's live EL DÍA (the 25 Aug build; this login also holds `74279c94…` from 18 Aug); `bf758d00…` (18 Aug build, live 6–10 Sep) · `24566e64…` (27 Aug) belong to other logins and are dead here — artifacts die with account switches. Before ever rebuilding anew: `Artifact read` the url, on not-found run `Artifact list` and reuse the account's own EL DÍA if one exists). URGENCIA items carry the ultra-mini red dot; INTENTIONAL items stay clean. **The FLOW line carries design WEIGHT and a CONCRETE HOUR** — larger type, the hour in mono accent, "no negociable" (Marc, 17 Aug: *"el flow hay que schedulearlo — si no se schedulea, me lo salto… ocurre a esta hora, no es negociable; todo lo demás ocurre alrededor"*). Footer stamp updated (`date` first).
