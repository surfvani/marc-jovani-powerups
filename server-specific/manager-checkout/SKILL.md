...
- Board **MAÑANA bar** + **`dia-card.html`** → redeploy the "EL DÍA" artifact, SAME url (https://claude.ai/code/artifact/24566e64-8834-4150-998d-75217873eb3b — rebuilt 27 Aug 2026 at the tower migration; `bf758d00-…`, `74279c94-…` and `0bcde176-…` are dead — artifacts die with account switches). URGENCIA items carry the ultra-mini red dot; INTENTIONAL items stay clean. **The FLOW line carries design WEIGHT and a CONCRETE HOUR** — larger type, the hour in mono accent, "no negociable" (Marc, 17 Aug: *"el flow hay que schedulearlo — si no se schedulea, me lo salto… ocurre a esta hora, no es negociable; todo lo demás ocurre alrededor"*). Footer stamp updated (`date` first).
