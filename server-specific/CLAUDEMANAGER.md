...
| **THE INBOX — where Marc's captures land.** Ideas ("IDEA: …"), improvement notes ("IMPROVEMENT: …"), decisions logged for cross-agent handoff, session conclusions he wants the next agent to inherit. **There is exactly ONE inbox and it lives INSIDE the board folder. NEVER create another one anywhere — least of all at the NORTH_STAR root** (15 Aug 2026: a manager did, and the next morning's manager could not find the file). Every file in it opens with a **"What this is:"** header carrying its own capture protocol and consumption moment (`CA_IMPROVEMENTS.md` → surface IN FULL at W3 open; `CA_WEBINAR_IDEAS.md` → at webinar prep, 24 Aug) — **read that header before appending**, then append verbatim, dated, tagged; never paraphrase, never delete. **When Marc says "log it" or asks "inbox? project? board? I don't know" — THIS is the answer: `ls` it, put it here, and tell him that is where it went.** No existing file fits → a new dated `YYYY-MM-DD_slug.md` in this same folder. **Always cite it by FULL path** in the digest and in your log — a bare `inbox/…` sends the next manager hunting | `/home/ubuntu/NORTH_STAR/BOARDS/PROJECT_HERO/inbox/` · consumed items → `inbox/processed/` |
