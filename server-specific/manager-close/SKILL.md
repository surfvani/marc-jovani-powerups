...
**POWER OFF (28 Aug 2026).** `🔻 THE BOX IS POWERING OFF` on the `mgr` line = box held open **15 min** while you write (was 10; the s1 farewell close measured >10 on a fat session). **LEAN form, same as the emergency rotate: Steps 1 → 3 → 6 → 7 → 8** — skip the board page unless stale, skip WINS. **NO marker, NO kill** — the hook sees your close SHA move and does both (reason `shutdown`); next boot takes over silently like the 2am rotate. Miss the window → successor inherits a confession + `mgr-tail` instead of your brief.
